async function loadStats() {
    ...
    if (document.getElementById('status-storno').checked) statusFilter.push('storniert');

    // 3. Wochentag-Filter
    const selectedWeekdays = Array.from(document.querySelectorAll('.wd-check:checked'))
                                  .map(c => parseInt(c.value));

    // 4. Menü-Produkt-Filter
    const activeProducts = Array.from(document.querySelectorAll('.prod-check:checked'))
                                .map(p => p.value);

    // --- WICHTIG: Hier oben deklarieren, damit sie überall in der Funktion bekannt ist ---
    let filteredData = []; 

    try {
        const response = await fetch(`/api/get-all-orders?von=${von}&bis=${bis}&_t=${Date.now()}`);
