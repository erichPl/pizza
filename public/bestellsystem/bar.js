...
let tv="";let currentLoadedOrders=[];setBon(false);let pwInput;sessionStorage.setItem('userRole','kellner');let zeigePreise=false;let currentPassword="";let selectedOrderIds=new Set();let aktuellerStatus={};const tageNamen=["Sonntag","Montag","Dienstag","Mittwoch","Donnerstag","Freitag","Samstag"];function toggleSelection(orderId){if(selectedOrderIds.has(orderId)){selectedOrderIds.delete(orderId);}else{selectedOrderIds.add(orderId);}}
