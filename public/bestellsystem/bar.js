...
let currentLoadedOrders=[];setBon(false);let pwInput;sessionStorage.setItem('userRole','kellner');let zeigePreise=false;let currentPassword="";let selectedOrderIds=new Set();let aktuellerStatus={};function toggleSelection(orderId){if(selectedOrderIds.has(orderId)){selectedOrderIds.delete(orderId);}else{selectedOrderIds.add(orderId);}}
setInterval(()=>{document.getElementById('clock').innerText=new Date().toLocaleTimeString('de-DE');},1000);let lastOrderCount=0;const notificationSound=new Audio('/bestelle1/notification.mp3');const feedbackSound=new Audio('https://actions.google.com/sounds/v1/foley/drawbridge_teal.ogg');async function fetchOrders(){const savedPw=sessionStorage.getItem('barPassword');const orderId=localStorage.getItem('orderId');if(!savedPw){if(document.getElementById('barLoginOverlay')){document.getElementById('barLoginOverlay').style.display='flex';}
return;}
...
async function fetchHistory5(){const von=document.getElementById('filterVon').value;const bis=document.getElementById('filterBis').value;alert(von+" "+bis);if(document.getElementById('historyFilter').style.display==='none')return;if(!currentPassword){currentPassword=sessionStorage.getItem('key_admin');}
...
const neueZeiten={};for(let i=0;i<7;i++){const startVal=document.getElementById(`start-${i}`).value;const endeVal=document.getElementById(`ende-${i}`).value;const istZu=document.getElementById(`zu-${i}`).checked;neueZeiten[i]={name:tageNamen[i],start:startVal,ende:endeVal,zu:istZu};}
const payload={zeiten:neueZeiten,pw:pw};try{const response=await fetch('/api/admin/oeffnungszeiten',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});const result=await response.json();if(response.ok&&result.success){alert("✅ Gespeichert! Die Handys der Gäste wurden aktualisiert.");closeSettingsModal();}else{alert("❌ Fehler vom Server: "+(result.error||"Unbekannter Fehler"));}}catch(err){console.error("Netzwerkfehler:",err);alert("❌ Verbindung zum Server fehlgeschlagen.");}}
...
function befuelleModal(config){if(!config)return;const tage=config.wochentage;if(tage){for(let i=0;i<7;i++){const tagDaten=tage[i.toString()]||tage[i];if(tagDaten){if(document.getElementById(`start-${i}`))document.getElementById(`start-${i}`).value=tagDaten.start;if(document.getElementById(`ende-${i}`))document.getElementById(`ende-${i}`).value=tagDaten.ende;if(document.getElementById(`zu-${i}`))document.getElementById(`zu-${i}`).checked=tagDaten.zu;}}}
const vorlaufInput=document.getElementById('input-vorlauf');if(vorlaufInput&&config.timerStartVorlauf!==undefined){vorlaufInput.value=config.timerStartVorlauf;console.log("Timer-Wert erfolgreich geladen:",config.timerStartVorlauf);}}
...
function updateBarTimer(){const timerBalken=document.getElementById('timer-balken');const timerAnzeige=document.getElementById('timer-countdown');const infoStart=document.getElementById('info-start');const infoReg=document.getElementById('info-regulaer');const infoMan=document.getElementById('info-manuell');const manuellHinweis=document.getElementById('manuell-hinweis');if(!timerBalken||!aktuellerStatus)return;const startZeit=aktuellerStatus.heute?aktuellerStatus.heute.start:"--:--";const regEnde=aktuellerStatus.heute?aktuellerStatus.heute.ende:"--:--";const manEnde=aktuellerStatus.bestellStopManuell;const vorlauf=aktuellerStatus.timerStartVorlauf||30;if(infoStart)infoStart.innerText=startZeit;if(infoReg)infoReg.innerText=regEnde;if(manEnde){if(infoMan)infoMan.innerText=manEnde;if(manuellHinweis)manuellHinweis.style.display='inline';}else{if(manuellHinweis)manuellHinweis.style.display='none';}
const zielZeitStr=manEnde||(aktuellerStatus.heute?aktuellerStatus.heute.ende:null);if(!zielZeitStr||zielZeitStr==="--:--"){timerBalken.style.display='none';return;}
const jetzt=new Date();const[stunden,minuten]=zielZeitStr.split(':');const zielDatum=new Date();zielDatum.setHours(parseInt(stunden),parseInt(minuten),0,0);const diff=zielDatum-jetzt;const gesamtSekunden=Math.floor(diff/1000);if(diff>0){const minutenRest=Math.floor(gesamtSekunden/60);const sekundenRest=gesamtSekunden%60;if(minutenRest<vorlauf){timerBalken.style.display='block';if(minutenRest<5){timerBalken.style.backgroundColor='#c0392b';}else{timerBalken.style.backgroundColor='#e67e22';}
const displayMin=minutenRest<10?'0'+minutenRest:minutenRest;const displaySek=sekundenRest<10?'0'+sekundenRest:sekundenRest;timerAnzeige.innerText=`${displayMin}:${displaySek}`;}else{timerBalken.style.display='none';}}else{timerBalken.style.display='block';timerBalken.style.backgroundColor='#2c3e50';timerAnzeige.innerText="BESTELL-STOP AKTIV";}}
socket.on('status-update',(data)=>{console.log("Bar-Ansicht: Status erhalten",data);aktuellerStatus=data;updateBarTimer();});setInterval(updateBarTimer,1000);async function initBar(){const res=await fetch('/api/status');aktuellerStatus=await res.json();updateBarTimer();}
