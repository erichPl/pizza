...
if(!heute){console.error("KRITISCHER FEHLER: Datenstruktur unbekannt!");console.log("Inhalt von aktuellerStatus:",JSON.stringify(aktuellerStatus));return;}
const jetzt=new Date();const jetztHHMM=jetzt.getHours().toString().padStart(2,'0')+":"+
jetzt.getMinutes().toString().padStart(2,'0');const startZeit=heute.start||"00:00";const regEnde=heute.ende||"00:00";const schlussZeit=aktuellerStatus.bestellStopManuell||regEnde;const istGeschlossen=heute.zu===true||heute.zu==="true"||aktuellerStatus.zustand!=='offen';let ende=regEnde;if(schlussZeit>regEnde){ende=regEnde+" - Heute Verlängerung bis "+schlussZeit;}
