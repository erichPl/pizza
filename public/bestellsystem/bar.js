...
let tv="";let currentLoadedOrders=[];setBon(false);let pwInput=localStorage.getItem('barPassword')||sessionStorage.getItem('barPassword')||"";sessionStorage.setItem('userRole','kellner');let zeigePreise=false;let currentPassword="";let selectedOrderIds=new Set();let aktuellerStatus={};const tageNamen=["Sonntag","Montag","Dienstag","Mittwoch","Donnerstag","Freitag","Samstag"];function toggleSelection(orderId){if(selectedOrderIds.has(orderId)){selectedOrderIds.delete(orderId);}else{selectedOrderIds.add(orderId);}}
setInterval(()=>{document.getElementById('clock').innerText=new Date().toLocaleTimeString('de-DE');},1000);let lastOrderCount=0;const notificationSound=new Audio('notification.mp3');const feedbackSound=new Audio('https://actions.google.com/sounds/v1/foley/drawbridge_teal.ogg');async function fetchOrders(){const historyEl=document.getElementById('historyFilter');const isHistoryVisible=historyEl&&historyEl.style.display!=='none'&&historyEl.style.display!=='';if(isHistoryVisible){console.log("Historie ist offen - automatischer Live-Update gestoppt.");return;}
