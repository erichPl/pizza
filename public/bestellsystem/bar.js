...
let currentLoadedOrders=[];setBon(false);sessionStorage.setItem('userRole','kellner');let zeigePreise=false;let currentPassword="";let selectedOrderIds=new Set();function toggleSelection(orderId){if(selectedOrderIds.has(orderId)){selectedOrderIds.delete(orderId);}else{selectedOrderIds.add(orderId);}}
...
async function submitBarLogin(){const pwInput=document.getElementById('barPasswordInput').value;sessionStorage.setItem('barPassword',pwInput);sessionStorage.setItem('userRole','kellner');document.getElementById('barLoginOverlay').style.display='none';fetchOrders();}
